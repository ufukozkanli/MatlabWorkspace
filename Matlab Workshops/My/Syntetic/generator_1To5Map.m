...
for t=1:inf;
    ...
    for r=1:IMSIZE(1)
        for c=1:IMSIZE(2)
            COEF = PIX_COEF(r,c);
            %DELTA_HOR = COEF.Sx + (sin(2*pi * c/IMSIZE(2)*5) + sin(2*pi * r/IMSIZE(1)*3) ) * COEF.Ax ;% Generate Sine Wave
            %DELTA_VER = COEF.Sy + (sin(2*pi * c/IMSIZE(2)*3) + sin(2*pi * r/IMSIZE(1)*5) ) * COEF.Ay ; % Generate Sine Wave
            DELTA_HOR = COEF.Sx + (sin(2*pi * c/IMSIZE(2)) + sin(2*pi * r/IMSIZE(1)) ) * COEF.Ax ;% Generate Sine Wave
            DELTA_VER = COEF.Sy + (sin(2*pi * c/IMSIZE(2)) + sin(2*pi * r/IMSIZE(1)) ) * COEF.Ay ; % Generate Sine Wave
            %DELTA_HOR = 0;
            if(abs(DELTA_HOR)<.5)             
                DELTA_HOR=DELTA_HOR + 0.5*sign(DELTA_HOR);    
            end
            if( abs(DELTA_VER)<.5)             
                DELTA_VER=DELTA_VER + 0.5*sign(DELTA_VER);              
            end
            
            
            
            VEL(r,c).Vx=DELTA_HOR/DT ;
            %* BOXCOEFF(2);
            VEL(r,c).Vy=DELTA_VER/DT ;                       
            %* BOXCOEFF(1);
    ...
    CENTER=3;
   if(t==1)
        IMG2_VEC =   repmat(struct('Vx',0,'Vy',0), IMSIZE(1).*5, IMSIZE(2).*5 );
        for i=1:IMSIZE(1)
            for j=1:IMSIZE(2)
                IMG2_VEC((i-1)*5+3,(j-1)*5+3).Vx=VEL(i,j).Vx;
                IMG2_VEC((i-1)*5+3,(j-1)*5+3).Vy=VEL(i,j).Vy;
            end
        end

        %cen_IMG2_VEC=cell2mat(struct2cell(IMG2_VEC(CENTER:WIDTH:end,CENTER:WIDTH:end)));
        IMG2_VEC_mat = cell2mat(struct2cell(IMG2_VEC(:,:)));
        cen_IMG2_VEC = IMG2_VEC_mat(:,CENTER:WIDTH:end,CENTER:WIDTH:end);
    
         hFig=figure(3);
         clf
        ncquiverref(CENTER:WIDTH:IMSIZE(2)*5-(WIDTH-1),CENTER:WIDTH:IMSIZE(1)*5-(WIDTH-1),reshape(cen_IMG2_VEC(1,:,1:end-1),size(cen_IMG2_VEC,2),size(cen_IMG2_VEC,3)-1),reshape(cen_IMG2_VEC(2,:,1:end-1),size(cen_IMG2_VEC,2),size(cen_IMG2_VEC,3)-1),'m/s','max','true','col',[1:1:MAXVEL])    
    end    
    WIDTH =9;
    CENTER=5;
...
P=PARTICLES(:,pindx);
try
    DELTA_HOR = (VEL(P(1),P(2)).Vx * DT )/SUBPIXEL_W;
DELTA_VER = (VEL(P(1),P(2)).Vy * DT )/SUBPIXEL_W;
catch(e)
e
end
...
PARTICLES(1,pindx) = P(1)+ round(DELTA_VER);
PARTICLES(2,pindx) = P(2)+ round(DELTA_HOR);
...
%DELTA_HOR = COEF.Sx + (sin(2*pi * t)) * COEF.Ax + COEF.Nx ;% Generate Sine Wave
%DELTA_VER = COEF.Sy + (sin(2*pi * t)) * COEF.Ay + COEF.Ny; % Generate Sine Wave
%DELTA_HOR = COEF.Sx + (sin(2*pi * c/IMSIZE(2)*5) + sin(2*pi * r/IMSIZE(1)*3) ) * COEF.Ax ;% Generate Sine Wave
DELTA_HOR = COEF.Sx + (sin(2*pi * P(2)/IMSIZE(2)*5) + sin(2*pi * P(1)/IMSIZE(1)*3) * rand()) * COEF.Ax  ;% Generate Sine Wave
DELTA_VER = COEF.Sy + (sin(2*pi * P(2)/IMSIZE(2)*3) + sin(2*pi * P(1)/IMSIZE(1)*5) * rand()) * COEF.Ay; % Generate Sine Wave
